...
def detection(image, algorithm)
  case algorithm 
  when 0:
    points = CamKeypoints.new(100000) 
    start = Time.now
    image.to_yuv.fast_hessian_detector(points, 90, CAM_UPRIGHT)
    stop = Time.now
    duration = stop - start
    return [points, duration]
  when 1:
    return surf_detector(image.to_y, "-thres 11000 -u")
  when 2:
    return sift_detector(image.to_y)
  end
end  
...
def bench_myobjects
  ...
  width = [720, 690, 500]
  height = [640, 910, 500]
  algorithm_names = ['CamKeypoints', 'surf', 'sift']
  ...
  for algorithm in 0..2 do
    ...
    (0...model_images.size).each do |i|
      ...
      points, duration = detection(image, algorithm)
	...
	#puts "#{matches.nb_matches}x#{matches.nb_outliers}"
	...
	dimage.draw_line(uv[6].x, uv[6].y, uv[4].x, uv[4].y, 0)
      ...
      # Save result image
      dimage.save_bmp("../../output/#{test_images[i]}_#{algorithm_names[algorithm]}.bmp")	
    end

    puts "# features in scenes = #{nb_features}"
    puts "# matching features = #{score} (#{score * 100.0 / nb_features})"
    puts "Execution time = #{total_duration}"
  end
